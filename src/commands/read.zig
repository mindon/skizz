...
pub fn execute(allocator: std.mem.Allocator, _: std.Io, args: []const []const u8) !void {
    if (args.len == 0) {
        io_helper.printErr("{s}Error:{s} Missing skill name\n", .{ Color.red, Color.reset });
    ...
    if (skill_loc == null) {
        io_helper.printErr("{s}Error:{s} Skill '{s}' not found\n", .{ Color.red, Color.reset, skill_name });
        io_helper.printErr("\nSearched:\n", .{});
        io_helper.printErr("  .agent/skills/ (project universal)\n", .{});
        io_helper.printErr("  ~/.agent/skills/ (global universal)\n", .{});
        io_helper.printErr("  .claude/skills/ (project)\n", .{});
        io_helper.printErr("  ~/.claude/skills/ (global)\n", .{});
    ...
    const content = skills_util.readFileAlloc(allocator, loc.path) catch |err| {
        io_helper.printErr("{s}Error:{s} Failed to read skill file: {any}\n", .{ Color.red, Color.reset, err });
        std.process.exit(1);
    };
    defer allocator.free(content);

    // Output in Claude Code format
    io_helper.print("Reading: {s}\n", .{skill_name});
    io_helper.print("Base directory: {s}\n", .{loc.base_dir});
    io_helper.print("\n", .{});
    io_helper.writeStdout(content);
    if (content.len > 0 and content[content.len - 1] != '\n') {
        io_helper.print("\n", .{});
    }
    io_helper.print("\n", .{});
    io_helper.print("Skill read: {s}\n", .{skill_name});
}
